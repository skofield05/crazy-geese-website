...
- **Finale-Flyer ausgebessert** – das Platzhalter-Schild mit dem Fragezeichen und die Zeile „GEGNER WIRD NOCH BEKANNT GEGEBEN!" waren nach der Auslosung falsch. Statt eines Neuentwurfs pixelgenau in die Vorlage retuschiert (Pillow):
  - **Rechtes Wappen ersetzt** durch das echte Danube-Titans-Logo (Quelle: `danube-titans.at`, 1182x1182 PNG mit Alpha, auf 186 px skaliert). Der Bereich wird vorher mit der Box-Hintergrundfarbe (#00060E + leichtem Rauschen) uebermalt, weil das alte Schild breiter war als der neue Kreis ihn deckt. **Der rote Banner-Pinselstrich wird danach zeilenweise aus dem Original zurueckkopiert** (pro Spalte ab dem obersten roten Pixel), sonst haette das Uebermalen seine ausgefranste Oberkante mitgenommen.
  - **Logo-Groesse bewusst 186 px statt 214 px** (= Geese-Logo): Der Geese-Kreis darf unten hinter dem Banner verschwinden, weil dort nur der leere weisse Ring liegt. Beim Titans-Logo steht dort „BASEBALL" im Ring – bei gleicher Groesse haette der Banner das Wort abgeschnitten und wie ein Fehler ausgesehen. Unterkante jetzt bei y=665, genau ueber der Banner-Oberkante.
  - **Bannerzeile** neu: „GEGNER: DANUBE TITANS" in Arial Narrow Bold Italic (30 px, naechster Verwandter der Original-Schrift). Der alte Text wurde nicht mit Rot uebermalt, sondern **zeilenweise horizontal interpoliert** – der Pinselstrich hat Textur und Verlauf, ein Flaechen-Fill waere als Rechteck sichtbar geblieben.
- **Dateien umbenannt** auf `img/finale-2026-titans[-thumb].jpg` (318 KB / 134 KB, gleiche Groessen wie vorher). Bilder haben **keinen Cache-Buster** – unter dem alten Namen haetten alle, die den Flyer schon gesehen haben, bis zu 10 Minuten (GitHub Pages `max-age=600`) und je nach Browser laenger die Fragezeichen-Version behalten. Alte Dateien geloescht, `bild`/`bild_full`/`bild_alt` in `data.json` nachgezogen.
- Verifiziert: 9 Assertions (Thumb-Pfad + 640x800 geladen, Seitenverhaeltnis unveraendert, Lightbox oeffnet das 1080er Grossformat unter dem neuen Namen, `alt` nennt die Titans und nicht mehr „noch bekannt"), Validator 0 Fehler.

### 2026-09-09
- **Playoff-Paarungen stehen fest** (Liga-Spielplan): Das **Finale** am 13.09., 14:00 am Geese Ballpark geht gegen die **Danube Titans**; das **Spiel um Platz 3** um 11:00 ist **Schremser Beers 2 vs Vienna Bucks**. Beide Eintraege in `data/data.json` von Hand aufgeloest, statt auf den naechsten Scraper-Lauf zu warten.
- **Finale (#FINALE):** `gast` gesetzt, `platzhalter` entfernt und **`spielnr_fest: true`** gesetzt. Das Flag ist hier load-bearing: ABF fuehrt das Finale als **#80**, und ohne den Schutz haette Match-Stufe 2 (heim+gast+Datum) beim naechsten Lauf die `spielnr` ueberschrieben – die ICS-UID `cg-FINALE@crazy-geese.at` waere gewandert und jeder, der den Termin schon importiert hat, haette ein zweites Event im Kalender. Verifiziert: UID unveraendert.
- **Spiel um Platz 3 (#79):** Teams eingetragen, `fremdspiel: true` bleibt (keine Geese-Beteiligung → OHNE-GEESE-Badge, raus aus Highlight-Karten, „Letzte Ergebnisse", JSON-LD und beiden ICS). Der `hinweis` verliert den jetzt veralteten Satz „Die Paarung steht nach den Halbfinals am 06.09. fest."
- **Nicht eingetragen:** die zwei Platzierungsspiele am 12.09. in AltWeitra (Lawnmowers–Metrostars 3, Red Devils–Ravens). Weder Geese-Beteiligung noch unser Ballpark – die haben auf der Seite nichts verloren. Die Geese sind dort auch nicht als Umpire/Scorer eingeteilt (das sind sie nur beim 11:00-Spiel bei uns).
- Verifiziert: 17 Assertions (Finale-Karte mit echtem Gegner, kein Platzhaltertext mehr im HTML, Platz 3 nur im Spielplan mit OHNE-GEESE-Badge, JSON-LD enthaelt am 13.09. **nur** das Finale, Flyer-Seitenverhaeltnis unveraendert), 4 Viewports (320–1280) ohne Overflow. `validate_data.py` 0 Fehler/0 Warnungen, beide ICS regeneriert (je 1 Event, UID stabil).
- **Offen:** Der Finale-Flyer (`img/finale-2026*.jpg`) sagt weiterhin „Gegner wird noch bekannt gegeben" – ein aktualisiertes Bild waere jetzt moeglich (Feld `bild`/`bild_full`/`bild_alt` am Spiel).
